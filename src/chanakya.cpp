...
int main(){
	...
	while(true) {
		...
		while(true) {
			...
			/* If no matching child found, provide a default response */
			if(maxScore == -1000) {
				cout<<name<<": I'm sorry, I don't understand that question. Try asking about matches, team, stats, or help."<<endl;
				cout.flush();
				plugins.displayAnswers(sentence,name,false);
				cout<<endl;
				break;
			}
			...
			if(current_node.children.size() == 0) {
				cout<<name<<": "<<current_node.value()<<"."<<endl; /* Print the lead node value. */
